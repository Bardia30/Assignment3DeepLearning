...
from ee8223.train_results import FitResult
...
def cnn_experiment(
    ...
    model_type="cnn",
    pooling_type="max",
    pooling_params=None,  # Allow pooling parameters to be passed
    **kw,
):
    """
    ...
    # Ensure pooling_params includes kernel_size
    if pooling_params is None:
        pooling_params = {}
    pooling_params.setdefault("kernel_size", 2)  # Set default kernel_size if not provided
    ...
    model = model_cls(
        ...
        pooling_params=pooling_params,  # Pass pooling_params with kernel_size
    )
    ...
    test_loader = DataLoader(ds_test, batch_size=bs_test)

    # Train the model using ClassifierTrainer
    trainer = ClassifierTrainer(model, loss_fn, optimizer, device=device)
    fit_res = trainer.fit(
        train_loader, test_loader, num_epochs=epochs, checkpoints=checkpoints
    )
    # ========================
...
def save_experiment(run_name, out_dir, cfg, fit_res):
    ...
    # Corrected cfg_LK
    cfg_LK = f'L{cfg["layers_per_block"]}_K{cfg["filters_per_layer"][0]}'
